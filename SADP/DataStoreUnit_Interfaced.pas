...
const
    ...
    IID_ISettingsProvider: TGUID = '{69C2B9E0-A75C-4231-A392-9B0682FED244}';
    IID_IConnectionOwner: TGUID = '{333C25E3-95AC-4bf9-B96B-717D7D484633}';
...
type
    ...
    ISettingsProvider = interface(IInterface)
    	['{69C2B9E0-A75C-4231-A392-9B0682FED244}']
    	function GetIgnoreDirection: Boolean;
    end;

    TConnection = class;
    IConnectionOwner = interface(ISettingsProvider)
    	['{333C25E3-95AC-4bf9-B96B-717D7D484633}']
        function Remove(const SelfValue: TConnection): Integer;
    end;
	...
	public
		...
		constructor Create(const AKey: TNode; const AValue: TNode; const _Weight: Integer;
			const ConnectionFlags: TNodeDirection; const OwnerList: IConnectionOwner);
        destructor Destroy; override;
	end;

	TConnectionList = class(TObjectList<TConnection>, IConnectionOwner)
    private
    	fSettings: ISettingsProvider;
    protected
        FRefCount: Integer;
    ...
    public
        ...
        function GetIgnoreDirection: Boolean;
		constructor Create(Owner: ISettingsProvider); overload;
        constructor Create(Owner: ISettingsProvider; const AComparer: IComparer<TConnection>; AOwnsObjects: Boolean = True); overload;
    end;

	TConnOperation = (coAdd, coDelete);

    TPendingOpList = class;
	TPendingOp = class(TInterfacedObject, IConnectionOwner)
    private
    	fSettings: ISettingsProvider;
    public
        ...
        constructor Create(Owner: ISettingsProvider);
        destructor Destroy; override;
	end;

    TPendingOpList = class(TObjectList<TPendingOp>, ISettingsProvider)
    private
        fSettings: ISettingsProvider;
    protected
        FRefCount: Integer;
	...
	TDataStore = class(TInterfacedObject, ISettingsProvider)
	private
...
// TPendingOpList //
// ////////////// //
procedure TPendingOpList.AfterConstruction;
begin
// Release the constructor's implicit refcount
  InterlockedDecrement(FRefCount);
end;
procedure TPendingOpList.BeforeDestruction;
begin
  if RefCount <> 0 then
    System.Error(reInvalidPtr);
end;
class function TPendingOpList.NewInstance: TObject;
begin
  Result := inherited NewInstance;
  TConnectionList(Result).FRefCount := 1;
end;

function TPendingOpList.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
function TPendingOpList._AddRef: Integer;
begin
  Result := InterlockedIncrement(FRefCount);
end;
function TPendingOpList._Release: Integer;
begin
...
constructor TPendingOpList.Create(Owner: ISettingsProvider);
begin
    Owner.QueryInterface(IID_ISettingsProvider, fSettings);
    inherited Create(False); //Don't own the objects as they are RefCounted and self-freed on their own
end;
...
procedure TConnectionList.BeforeDestruction;
begin
  if RefCount <> 0 then
    System.Error(reInvalidPtr);
end;
...
class function TConnectionList.NewInstance: TObject;
begin
  Result := inherited NewInstance;
  TConnectionList(Result).FRefCount := 1;
end;
function TConnectionList.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := 0
  else
...
function TConnectionList._AddRef: Integer;
begin
  Result := InterlockedIncrement(FRefCount);
end;
function TConnectionList._Release: Integer;
begin
  Result := InterlockedDecrement(FRefCount);
  if Result = 0 then
    Destroy;
...
constructor TConnectionList.Create(Owner: ISettingsProvider);
begin
	Owner.QueryInterface(IID_ISettingsProvider, fSettings);
    inherited Create;
end;

constructor TConnectionList.Create(Owner: ISettingsProvider; const AComparer: IComparer<TConnection>; AOwnsObjects: Boolean = True);
begin
	Owner.QueryInterface(IID_ISettingsProvider, fSettings);
...
constructor TPendingOp.Create(Owner: ISettingsProvider);
begin
	Owner.QueryInterface(IID_ISettingsProvider, fSettings);
    inherited Create;
end;
...
begin
	Key := AKey;
	Value := AValue;
	OwnerList.QueryInterface(IID_IConnectionOwner, Owner);
	...
	begin
		...
		begin
			...
			begin
				...
				Connection := TConnection.Create(VertexKey, VertexValue, Weight, PO_Direction, PendingOp);
			end
			else
			begin
...
function TDataStore.GetIgnoreDirection: Boolean;
begin
